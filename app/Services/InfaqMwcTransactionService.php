...
class InfaqMwcTransactionService
{
    protected InfaqMwcTransactionRepository $repository;

    public function __construct(InfaqMwcTransactionRepository $repository)
    {
    ...
    {
        $last = InfaqMwcTransaction::where('transaction_code', 'like', 'INM%')->orderByDesc('id')->first();

        if (!$last || !$last->transaction_code) {
            return 'INM00001';
        }

        preg_match('/INM(\d+)/', $last->transaction_code, $matches);
        $lastNumber = isset($matches[1]) ? (int) $matches[1] : 0;
        $nextNumber = $lastNumber + 1;

        $digitLength = max(5, strlen((string) $nextNumber));
        return 'INM' . str_pad($nextNumber, $digitLength, '0', STR_PAD_LEFT);
    }
}
