...
<div class="max-w-3xl rounded-3xl border border-zinc-200 bg-white p-6 shadow-sm sm:p-8">
    <form action="{{ route('mwc.management-munfiq.update', $munfiq->id) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <select name="data_ranting_id" id="data_ranting_id" class="mt-1 block w-full rounded-2xl border border-zinc-200 bg-white px-4 py-2.5 text-sm outline-none ring-0 focus:border-green-300" required>
                ...
                @foreach($rantings as $ranting)
                    <option value="{{ $ranting->id }}" {{ old('data_ranting_id', $munfiq->data_ranting_id) == $ranting->id ? 'selected' : '' }}>{{ $ranting->nama_ranting }} (Kode: {{ $ranting->kode_ranting }})</option>
                @endforeach
            </select>
        ...
        <div class="flex items-center gap-4 pt-4">
            ...
            <a href="{{ route('mwc.management-munfiq.index') }}" class="rounded-2xl border border-zinc-200 bg-white px-6 py-2.5 text-sm font-semibold text-zinc-700 transition hover:bg-zinc-50">
                Batal
            </a>
        </div>
    </form>
