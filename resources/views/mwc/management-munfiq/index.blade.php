...
<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
    <div class="relative w-full sm:w-96">
        <form method="GET" action="{{ route('mwc.management-munfiq.index') }}">
    ...
    <div class="flex items-center gap-3">
        ...
        <a href="{{ route('mwc.management-munfiq.create') }}" class="flex items-center gap-2 rounded-2xl bg-green-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-green-800">
            ...
            <tbody class="divide-y divide-zinc-100">
                @forelse($munfiqs as $index => $munfiq)
                    <tr class="hover:bg-zinc-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4">{{ $munfiq->data_ranting->nama_ranting ?? '-' }}</td>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('mwc.management-munfiq.edit', $munfiq->id) }}" class="flex h-8 w-8 items-center justify-center rounded-xl bg-amber-50 text-amber-600 transition hover:bg-amber-100" title="Edit">
                                    <i class="fas fa-edit text-xs"></i>
                                </a>
                                <form id="delete-form-{{ $munfiq->id }}" action="{{ route('mwc.management-munfiq.destroy', $munfiq->id) }}" method="POST" class="inline">
                                    @csrf
                                    @method('DELETE')
...
<form id="bulk-delete-form" method="POST" action="{{ route('mwc.management-munfiq.bulk-delete') }}" style="display: none;">
    @csrf
    @method('DELETE')
</form>
