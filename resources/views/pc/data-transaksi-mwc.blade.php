...
@section('content')
    <div class="w-full space-y-8">
        ...
        <div class="bg-white border border-slate-200 rounded-3xl p-6 shadow-sm">
            <form action="{{ route('pc.data-transaksi-mwc') }}" method="GET" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {{-- Wilayah Filter --}}
                <div class="space-y-2">
                            ...
                            <tbody class="divide-y divide-slate-100">
                                @forelse($pengeluarans as $item)
                                    <tr class="hover:bg-slate-50/50 transition-colors mwc-row" data-search="{{ strtolower($item->transaction_code . ' ' . $item->infaq_type) }}">
                                        ...
                                        <td class="px-6 py-5">
                                            <div class="font-bold text-red-700">Rp {{ number_format($item->gross_amount, 0, ',', '.') }}</div>
                                        </td>
                                        <td class="px-6 py-5">
                                            <div class="font-bold text-slate-900 leading-tight">{{ $item->penerima_manfaat }}</div>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="5" class="px-6 py-20 text-center text-slate-400">
                                            Belum ada data pengeluaran MWC.
                                        </td>
                                    </tr>
                                @endforelse
                            </tbody>
