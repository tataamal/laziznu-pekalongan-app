...
@section('page_title', 'Export Report Pentasarufan')
@section('page_subtitle', 'Preview and export pentasarufan data to Excel.')
...
@section('content')
    <div class="w-full space-y-6">
        ...
        <div class="bg-white border border-zinc-200 rounded-3xl shadow-sm overflow-hidden">
            ...
            <div class="p-6">
                <form action="{{ route('pc.export-report.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                    <div>
                    ...
                    <div>
                        <label for="wilayah_id" class="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Wilayah</label>
                    ...
                    <div>
                        <label for="source_type" class="block text-xs font-bold text-zinc-500 uppercase tracking-wider mb-2">Jenis Pentasarufan</label>
                        <select name="source_type" id="source_type" 
                                class="w-full rounded-2xl border-zinc-200 focus:border-green-500 focus:ring-green-500 text-sm py-2.5">
                            <option value="semua" {{ request('source_type') == 'semua' ? 'selected' : '' }}>Semua</option>
                            <option value="koin_nu" {{ request('source_type') == 'koin_nu' ? 'selected' : '' }}>Koin NU</option>
                            <option value="infaq_lainnya" {{ request('source_type') == 'infaq_lainnya' ? 'selected' : '' }}>Infaq Lainnya</option>
                        </select>
                    </div>
